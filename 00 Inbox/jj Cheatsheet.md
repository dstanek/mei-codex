...
[June 9, 2024](https://willhbr.net/archive#/2024/06/09/using-jj-for-the-version-control-operation-audit/) • [jj](https://willhbr.net/tags#jj) [tools](https://willhbr.net/tags#tools)

So I [just wrote about](https://willhbr.net/2024/06/07/the-version-control-operation-audit/) the version control operations that I use day-to-day. My new favourite thing is [JJ](https://github.com/martinvonz/jj)—a git-compatible version control system that I’ve also [written about before](https://willhbr.net/2024/04/01/its-not-me-its-git/)—so I thought I would explain how each of these operations are done with JJ.
...
This depends on your workflow, but `jj git push --all` will upload every branch to your remote. I also use `jj git push -c @-` to create a new auto-named branch, and `jj git push -b 'glob:willhbr/push-*'` to upload every auto-named branch. These all sit behind convenient aliases [that I’ve mentioned before](https://willhbr.net/2024/05/26/some-hot-jj-tips/).
...
Of course you could just do `jj new` _anywhere_, make your edits, and then do `jj squash --into $change` to move the changes. This works from anywhere to anywhere[1](https://willhbr.net/2024/06/09/using-jj-for-the-version-control-operation-audit/#fn:citation-needed). This does run an increased risk of creating conflicts, but you should live dangerously every once in a while.
...
1. I’m pretty sure? I haven’t checked though. [↩](https://willhbr.net/2024/06/09/using-jj-for-the-version-control-operation-audit/#fnref:citation-needed)